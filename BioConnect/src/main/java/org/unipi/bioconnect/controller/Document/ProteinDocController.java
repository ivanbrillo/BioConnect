...
public class ProteinDocController {
    ...
    public List<TrendAnalysisDTO> getTrendAnalysisForPathway(
            @Parameter(
                    description = "The protein-related pathway for trend analysis", example = "Lipid metabolism",
    ...
    public List<PathwayRecurrenceDTO> getPathwayRecurrence(
            @Parameter(
                    description = "The subsequence of the protein to search", example = "ATGT",
                    required = true, schema = @Schema(type = "string")
            )
    ...
    public List<ProteinDocDTO> searchProtein(
            @Parameter(
                    description = "The unique ID of the protein to search", example = "P68871",
                    required = true, schema = @Schema(type = "string")
            )
    ...
    public List<ProteinDocDTO> getProteinsByPathwayAndLocation(
            @Parameter(
                    description = "The pathway to filter proteins by", example = "Lipid metabolism",
                    required = true, schema = @Schema(type = "string")
            ) @RequestParam String pathway,

            @Parameter(
                    description = "The subcellular location to filter proteins by", example = "Microsome membrane",
                    required = true, schema = @Schema(type = "string")
